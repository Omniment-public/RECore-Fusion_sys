...
WLAN=wlan0
CON_AP="ap-recore"            # 事前に nmcli で作成
...
sudo nmcli device set $WLAN managed yes
...
if [[ "$WLAN_MODE" == "0" ]]; then
    ...
    sudo nmcli device connect $WLAN
...
if [[ -z "$WLAN_STATE" ]]; then
    ...
    # 自動チャネル選択 (失敗時は ch6)
    best_ch=$(python3 /usr/local/bin/recore/files/wlan_autochannel.py) || best_ch=1
    if [[ $best_ch -ge 1 && $best_ch -le 13 ]]; then
        sudo nmcli connection modify $CON_AP wifi.channel "$best_ch"
        echo "[boot_recore] AP channel set to $best_ch"
    fi

    # STA無効化
    sudo nmcli device disconnect $WLAN
